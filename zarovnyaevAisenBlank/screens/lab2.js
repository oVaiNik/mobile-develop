...
export default function AdministrativeDivisionsScreen() {
  ...
  const fetchDivisions = async (countryCode) => {
    try {
      const response = await fetch(`https://rawcdn.githack.com/kamikazechaser/administrative-divisions-db/master/api/${countryCode}.json`);
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", alignItems: "center" }}>
        <View style={{ margin: 20, flex: 1, justifyContent: "center", alignItems: "center" }}>
          <Text style={{ marginBottom: 10 }}>Административные единицы в {countries[countryIndex].name}:</Text>
